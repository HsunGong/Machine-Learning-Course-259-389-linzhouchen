function [Z,E] = ladmp_lrr(D,lambda,rho,DEBUG)
...
%clear global;
clearvars -global M;
global M;% M=Z_k+D'*(D-D*Z_k-E_{k+1}+Y/mu_k)/eta.
...
eta = norm2X*norm2X*1.02;%eta needs to be larger than ||D||_2^2, but need not be too large.
...
while iter<maxIter
    ...
    E = solve_l1l2(D - XZ + Y/mu,lambda/mu);
    
    %-----------Using PROPACK--------------%
    M = Z + D'*(D - XZ - E + Y/mu)/eta;
    ...
    XZ = D*Z;%introducing XZ to avoid computing D*Z multiple times, which has O(n^3) complexity.
    dY = D - XZ - E;
